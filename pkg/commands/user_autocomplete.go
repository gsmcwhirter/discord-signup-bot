...
func (c *UserCommands) autocompleteOpenEvents(ix *cmdhandler.Interaction, opts []entity.ApplicationCommandInteractionOption, focused entity.ApplicationCommandInteractionOption) ([]entity.ApplicationCommandOptionChoice, error) {
	ctx, span := c.deps.Census().StartSpan(ix.Context(), "userCommands.autocompleteOpenEvents", "guild_id", ix.GuildID().ToString())
	defer span.End()
...
func (c *UserCommands) autocompleteEventRoles(ix *cmdhandler.Interaction, opts []entity.ApplicationCommandInteractionOption, focused entity.ApplicationCommandInteractionOption) ([]entity.ApplicationCommandOptionChoice, error) {
	ctx, span := c.deps.Census().StartSpan(ix.Context(), "userCommands.autocompleteEventRoles", "guild_id", ix.GuildID().ToString())
	defer span.End()
