...
func (c *AdminCommands) closeInteraction(ix *cmdhandler.Interaction, opts []entity.ApplicationCommandInteractionOption) (cmdhandler.Response, []cmdhandler.Response, error) {
	ctx, span := c.deps.Census().StartSpan(ix.Context(), "adminCommands.closeInteraction", "guild_id", ix.GuildID().ToString())
	defer span.End()

	r := &cmdhandler.SimpleEmbedResponse{}

	logger := logging.WithMessage(ix, c.deps.Logger())
	level.Info(logger).Message("handling admin interaction", "command", "close")
	...
	if err := c.close(ctx, ix.GuildID(), eventName); err != nil {
		return r, nil, errors.Wrap(err, "could not close event")
	}

	level.Info(logger).Message("trial closed", "trial_name", eventName)
	r.Description = fmt.Sprintf("Event %q closed successfully", eventName)
	r.SetColor(okColor)

	return r, nil, nil
}

func (c *AdminCommands) closeHandler(msg cmdhandler.Message) (cmdhandler.Response, error) {
	ctx, span := c.deps.Census().StartSpan(msg.Context(), "adminCommands.closeHandler", "guild_id", msg.GuildID().ToString())
	defer span.End()
	...
	if err := c.close(ctx, msg.GuildID(), trialName); err != nil {
...
func (c *AdminCommands) close(ctx context.Context, gid snowflake.Snowflake, eventName string) error {
	ctx, span := c.deps.Census().StartSpan(ctx, "adminCommands.close", "guild_id", gid.ToString())
	defer span.End()
	...
	if err = t.SaveTrial(ctx, trial); err != nil {
		return errors.Wrap(err, "could not close event")
	}
